	...
	public void scheduler() {
		...
		for (int i = 0; i < sourceConfList.size(); i++) {
			...
			if (sourceConfList.get(i).getName().equalsIgnoreCase("Facebook")) {
				...
				try {
					rst = Unirest
							.get(staturl + sourceConfList.get(i).getName())
							.asString();
				} catch (UnirestException e) {
					e.printStackTrace();
				}
				...
				JsonParser parser = new JsonParser();
				JsonObject jo = parser.parse(rst.getBody()).getAsJsonObject();
					...
					if (modelList.size() > 0) {
						...
						if (rs.getStatus() == 200) {
							for (int j = 0; j < modelList.size(); j++) {
								...
								ProcessedModel processedModel = ProcessedModel.builder()
										.type(modelList.get(j).getType()).data(modelList.get(j).getData())
										.time(modelList.get(j).getTime()).apikey(modelList.get(j).getApikey())
										.label(modelList.get(j).getLabel()).build();
								processedModelRepository.save(processedModel);
							}
						} else {
							for (int k = 0; k < modelList.size(); k++) {
